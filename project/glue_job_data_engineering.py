...
args = getResolvedOptions(sys.argv, 
                          ['JOB_NAME', 'DATE', 'SOURCE_BUCKET', 'TARGET_BUCKET',
                           'CRAWLER'])
DATE = args['DATE']
INPUT_S3_URI = 'S3://{}/{}'.format(args['SOURCE_BUCKET'], DATE)
DATA_LAKE_BUCKET = args['TARGET_BUCKET']
(PUB_YEAR, PUB_MONTH, PUB_DAY) = DATE.split('-')
OUTPUT_S3_RAW_URI = 's3://{}/eurex/raw/pub_year_{}/pub_month_{}/pub_day_{}'.format(
        DATA_LAKE_BUCKET, PUB_YEAR, PUB_MONTH, PUB_DAY)
OUTPUT_S3_ML_URI = 's3://{}/eurex/data'.format(DATA_LAKE_BUCKET)
...
valid_df.coaleace(100).write.mode('overwrite').csv(OUTPUT_S3_ML_URI+'/valid', header=True)
...
logger.info('Starting to crawl the data in the data lake with crawler: {}'.
            format(CRAWLER))
